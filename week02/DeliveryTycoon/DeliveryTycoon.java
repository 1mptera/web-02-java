            ...
            while (!end) {
                ...
                if (choice == 1) {
                    ...
                    for (int i = 0; i < 5; i += 1) {
                        ...
                        int[] orderLists = new int[numberOfOrders];

                        for (int j = 0; j < numberOfOrders; j += 1) {

                            int food = random.nextInt(foodMenus.length);

                            for (int k = 0; k < j; k += 1) {

                                if (food == orderLists[k]) {
                                    j -= 1;
                                    break;
                                }
                        ...
                        System.out.println("체력: " + hp + ", 자산: " + money + "만원, 인지도: " + popularity);
                        System.out.println("--------------------------------------------");
                        System.out.println(numberOfOrders + "개의 주문이 들어 왔습니다. (10초 안에 음식을 완성해주세요.)");

                        for (int j = 0; j < numberOfOrders; j += 1) {
                            System.out.println(foodMenus[orderLists[j]]);
                        ...
                        for (int j = 0; j < numberOfOrders; j += 1) {
                            ...
                            if (answers[numberOfOrders - 1] == orderLists[numberOfOrders - 1]) {
